...
- Present the nine SEO capabilities as one bordered horizontal system on desktop. Every card starts compact; hovering a card or reaching its control with the keyboard expands it to the readable card width while the other eight contract. Keep the section height stable and never clip the selected card's description or list.
- At 900px and below, convert the capability system into one vertical disclosure group. A tap opens one complete capability at a time, a second tap closes it, and each control must expose its state through `aria-expanded`. If enhancement fails, retain the complete three/two/one-column card fallback so no service content is lost.
