...
const requiredDestinations = [
  ...
  '/wip#for=website-revamp', '/wip#for=website-design', '/services/free-website', '/services/seo', '/wip#for=geo', '/wip#for=copywriting', '/services/localization-and-translation', '/wip#for=maintenance',
...
for (const locale of ['pt-PT', 'es', 'el', 'ru', 'he']) {
  ...
  for (const key of ['Free Website', 'Website design', 'Explore website services', 'Copywriting', 'Filming', 'Photography', 'Video editing', 'Graphic design', 'Digital design', 'AI generation', 'Meta ads', 'Google ads', 'Influencer ads', 'SMEs', 'Events', 'Presential Events', 'Online Events', 'Health care', 'Local Business']) {
    assert.ok(data.strings[key], `${locale} is missing mega-menu translation: ${key}`);
  }
  assert.ok(data.services.itemLabels.freeAudit, `${locale} is missing the Free Audit service label`);
}
