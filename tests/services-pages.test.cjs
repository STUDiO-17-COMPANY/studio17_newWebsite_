...
test('website service-family page provides a distinct, translated decision journey', () => {
  ...
  assert.equal((html.match(/class="website-faq-list"[\s\S]*?<\/div><\/div><\/section>/)?.[0].match(/<details>/g) || []).length, 9);
...
test('free website page is transparent, lead-ready and translated in all site languages', () => {
  ...
  assert.doesNotMatch(html, />Design \+ development fee<\/p>/);
  assert.match(html, /class="free-value-reference"[\s\S]*?<s>€950<\/s>/);
  assert.match(html, /class="free-value-offer"[\s\S]*?<h2 id="free-value-title">€0<\/h2>/);
  assert.match(html, /data-service-key="fitAction"[\s\S]*?contact\?service=free-website/);
...
test('SEO page is an international, evidence-safe commercial service page', () => {
  ...
  assert.match(html, /class="seo-market-directory"[\s\S]*?href="\/seo\/cyprus"[\s\S]*?href="\/seo\/limassol"/);
  assert.equal((html.match(/class="seo-market-directory-grid"[\s\S]*?<\/div><\/div><\/section>/)?.[0].match(/class="seo-market-card/g) || []).length, 2);
...
test('SEO page translations cover the approved six-language scope', () => {
  ...
  for (const locale of ['el', 'ru']) {
    ...
    assert.match(page.locations, /href="\/seo\/cyprus"[\s\S]*?href="\/seo\/limassol"/, locale);
