...
(async () => {
  ...
  try {
    ...
    assert.ok((await launcher.boundingBox()).width <= 56, 'desktop launcher must begin as a compact icon');
    ...
    assert.ok((await mobileLauncher.boundingBox()).width <= 56, 'mobile launcher must remain compact');
