...
- `/` (`index.html`, rendered by `api/home-page.js`) — Figma-based homepage with a multilingual selected website-work showcase and the latest six published article links in the initial HTML.
...
- `/news` and `/news/page/<number>` (`news.html`, rendered by `api/news-page.js`) — server-rendered multilingual archive for Insights, Case Studies and News, with nine articles per page, metadata search and category filtering.
