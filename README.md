...
- `/llms.txt` (`api/llms.js`) — English plain-text LLM directory with curated core links and live published articles. See `LLMS.md`; shared head discovery metadata is generated by `scripts/sync-document-metadata.cjs` during builds.

- `/` (`home.template.html`, rendered by `api/home-page.js`) — Figma-based homepage with a multilingual selected website-work showcase and the latest six published article links in the initial HTML.
- `/sitemap` (`api/sitemap-template.html`, rendered by `api/sitemap-page.js`) — curated human-facing directory of the main published pages; detailed article, role and location discovery remains in `sitemap.xml` and `llms.txt`.
...
Install the small Vercel Runtime Cache dependency once, then start the local preview:
...
The homepage and News files are server templates so their article links exist in the initial response. Use the local server for those routes; the other static pages can still be opened directly when needed.
...
19. Update FAQ copy in all six canonical locale files, then regenerate `locales/locales.js` and update the XML sitemap in the same change. Keep the human sitemap limited to curated main-page destinations.
