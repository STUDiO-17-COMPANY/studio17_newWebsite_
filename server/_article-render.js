...
const { articleImageUrl } = require('./_article-image-url');
...
const renderBlocks = (blocks, tableLabel = UI.en.table, coverMarkup = '', imageVersion = '1') => {
  ...
  for (const block of blocks || []) {
    ...
    else if (block.type === 'image') html.push(`<figure class="article-inline-image"><img src="${escapeAttribute(articleImageUrl(block.imageId, 960, imageVersion))}" srcset="${escapeAttribute(articleImageUrl(block.imageId, 480, imageVersion))} 480w, ${escapeAttribute(articleImageUrl(block.imageId, 960, imageVersion))} 960w, ${escapeAttribute(articleImageUrl(block.imageId, 1600, imageVersion))} 1600w" sizes="(max-width: 800px) calc(100vw - 32px), 720px" width="1600" height="900" alt="${escapeAttribute(block.alt)}" loading="lazy" decoding="async">${block.caption ? `<figcaption>${escapeHtml(block.caption)}</figcaption>` : ''}</figure>`);
...
const renderCard = (item, locale) => `<article class="news-card"><a href="${escapeAttribute(getArticlePath(item, locale))}"><div class="news-image"><img src="${escapeAttribute(item.coverImage)}" alt="${escapeAttribute(item.coverAlt)}" width="720" height="405" loading="lazy" decoding="async"><span>${escapeHtml((CATEGORY_LABELS[locale] || CATEGORY_LABELS.en)[item.category] || item.category)}</span></div><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.summary)}</p><small>${escapeHtml(formatDate(item.publishedDate, locale))}&nbsp;&nbsp;•&nbsp;&nbsp;${escapeHtml(item.authorName)} | ${escapeHtml(item.authorRole)}</small></a></article>`;
const renderRailCard = (item, locale) => `<a class="article-rail-card" href="${escapeAttribute(getArticlePath(item, locale))}"><img src="${escapeAttribute(item.coverImage)}" alt="${escapeAttribute(item.coverAlt)}" width="720" height="405" loading="lazy" decoding="async"><span>${escapeHtml((CATEGORY_LABELS[locale] || CATEGORY_LABELS.en)[item.category] || item.category)}</span><h3>${escapeHtml(item.title)}</h3><small>${escapeHtml(formatDate(item.publishedDate, locale))}</small></a>`;
...
const renderArticleMain = article => {
  ...
  const managedCover = Boolean(article.coverImageId);
  const cover720 = managedCover ? articleImageUrl(article.coverImageId, 720, version) : article.coverImage;
  const cover1280 = managedCover ? articleImageUrl(article.coverImageId, 1280, version) : article.coverImage;
  const cover1600 = managedCover ? articleImageUrl(article.coverImageId, 1600, version) : article.coverImage;
  const coverSrcset = managedCover
    ? ` srcset="${escapeAttribute(cover720)} 720w, ${escapeAttribute(cover1280)} 1280w, ${escapeAttribute(cover1600)} 1600w"`
    : '';
  const authorVisual = article.authorImage
    ? `<img class="article-author-image" src="${escapeAttribute(article.authorImage)}" alt="" width="80" height="80" loading="lazy" decoding="async">`
  ...
  const coverMarkup = `<figure class="article-cover reveal"><img src="${escapeAttribute(cover1280)}"${coverSrcset} sizes="(max-width: 800px) calc(100vw - 32px), 720px" width="1600" height="900" alt="${escapeAttribute(content.coverAlt)}" loading="lazy" decoding="async">${content.coverCaption ? `<figcaption>${escapeHtml(content.coverCaption)}</figcaption>` : ''}</figure>`;
  return `<main id="article-content"><article data-i18n-skip>
    <header class="article-hero"><div class="hero-media" aria-hidden="true"><img src="${escapeAttribute(cover1600)}"${coverSrcset} sizes="100vw" width="1600" height="900" alt="" loading="eager" fetchpriority="high" decoding="async"></div><div class="shell article-hero-grid">
