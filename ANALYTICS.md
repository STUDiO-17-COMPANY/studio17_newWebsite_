...
- A translated Cookie settings control is added to the legal copy in every footer so the visitor can accept or withdraw consent later.
...
6. Open Cookie settings in the footer, reject Analytics and confirm the consent state changes to denied and `_ga` cookies are removed.
