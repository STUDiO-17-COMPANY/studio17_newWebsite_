...
- Removed individual articles, open roles, Cyprus/Limassol SEO landing pages, section anchors, WIP destinations and self-referencing Sitemap links from the page.
- Kept complete machine discovery in `sitemap.xml` and `llms.txt`, while making the human sitemap static, faster and independent of Google Drive or Careers availability.
- Updated all six sitemap locale descriptions and added regression coverage for the curated-page contract.

## 2026-09-26 — Responsive article image CDN pipeline

- Replaced raw Google Drive image delivery with versioned 160–1600 px WebP/JPEG variants generated by the existing article-image function.
- Added optional public Vercel Blob persistence with safe fallback to the existing Vercel CDN, one-year immutable caching and no additional Serverless Function.
- Added responsive `srcset`, high-priority hero loading, lightweight article-card imagery, deferred inline/author images and reserved image dimensions.
- Added image optimization regression coverage and documented the storage and cache contract.
