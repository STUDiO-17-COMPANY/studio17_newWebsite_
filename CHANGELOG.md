...
- Completed the revised About journey in Portuguese (Portugal), Spanish, Greek, Russian and Hebrew, including metadata, reviews, company and origin content, presentation controls and the careers CTA.
- Added regression coverage that fails whenever visible About copy lacks a translation in any supported non-English locale.
...
- Moved the human sitemap template behind the API route so Vercel cannot bypass dynamic link injection with a same-name static file.
...
- Refined every package description around the buyer and outcome, clarified that delivery periods are estimates and distinguished supplied-language implementation from separately quoted professional translation.
...
- Rebuilt the final Website Development CTA with the approved PHÓS Optics image, an improved conversion message and separate “Talk to sales” and “See our work” actions.
...
- Added accessible focus management, Escape and outside-click dismissal, mobile panel behavior, reduced-motion handling and regression coverage.
...
- Enlarged the Services CTA media window to a responsive 520 × 260px maximum, aligned it with the copy and retained a close subject-focused crop without increasing the section height.
